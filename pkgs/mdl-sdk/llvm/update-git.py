...
clang_update_script = get_file_revision(
    chromium_version, "tools/clang/scripts/update.py"
)
...
if get_current_revision() == commit["sha"]:
    ...
    sys.exit(0)
date = (
    datetime.fromisoformat(commit["commit"]["committer"]["date"].rstrip("Z"))
    .date()
    .isoformat()
)
...
hash = nix_prefetch_url(
    f'https://github.com/llvm/llvm-project/archive/{commit["sha"]}.tar.gz'
)
...
with fileinput.FileInput(DEFAULT_NIX, inplace=True) as f:
    for line in f:
        ...
        result = line
        result = re.sub(
            r'^      version = ".+";', f'    version = "{release_version}";', result
        )
        result = re.sub(
            r'^      rev = ".*";', f'      rev = "{commit["sha"]}";', result
        )
        result = re.sub(
            r'^      rev-version = ".+";', f'      rev-version = "{version}";', result
        )
